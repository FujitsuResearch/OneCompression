"""Threshold tests: Qwen3, scaling only / no rotation (rotation=False).
...
Usage::
    ...
    pytest tests/onecomp/pre_process/test_threshold_qwen3_scaling.py -v
"""
        ...
        for scale_mode in _VALID_SCALING_MODES:
            cases.append(
                pytest.param(
                    ...
                    id=f"qwen3-no_rot-scale_{scale_mode}-{fp}",
                )
            )
    return cases
...
class TestThresholdQwen3Scaling:
    """Verify scaling-only / no-rotation preserves Qwen3 output."""
