"""Threshold tests: TinyLlama, rotation+scaling, fp32_had=False.
...
Usage::
    ...
    pytest tests/onecomp/pre_process/test_threshold_tinyllama_rotation_scaling_fp16.py -v
"""
        ...
        for scale_mode in _VALID_SCALING_MODES:
            cases.append(
                pytest.param(
                    ...
                    id=f"tinyllama-rot_{rot_mode}-scale_{scale_mode}-fp16had",
                )
            )
    return cases
...
class TestThresholdTinyLlamaRotScaleFP16:
    """Verify rotation+scaling preserves TinyLlama output (fp32_had=False)."""
