"""Threshold tests: Qwen3, rotation+scaling, fp32_had=True.
...
Usage::
    ...
    pytest tests/onecomp/pre_process/test_threshold_qwen3_rotation_scaling_fp32.py -v
"""
        ...
        for scale_mode in _VALID_SCALING_MODES:
            cases.append(
                pytest.param(
                    ...
                    id=f"qwen3-rot_{rot_mode}-scale_{scale_mode}-fp32had",
                )
            )
    return cases
...
class TestThresholdQwen3RotScaleFP32:
    """Verify rotation+scaling preserves Qwen3 output (fp32_had=True)."""
