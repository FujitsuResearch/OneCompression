"""Threshold tests: TinyLlama, rotation only (rotation=True, scaling=False).
...
Usage::
    ...
    pytest tests/onecomp/pre_process/test_threshold_tinyllama_rotation.py -v
"""
        ...
        for rot_mode in _VALID_ROTATION_MODES:
            cases.append(
                pytest.param(
                    ...
                    id=f"tinyllama-rot_{rot_mode}-{fp}",
                )
            )
    return cases
...
class TestThresholdTinyLlamaRotation:
    """Verify rotation preserves TinyLlama output within threshold (rotation only)."""
